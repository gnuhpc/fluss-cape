...
public class DisableTableExecutor implements HBaseOperationExecutor {

    private static final Logger LOG = LoggerFactory.getLogger(DisableTableExecutor.class);

    private final TableStateManager stateManager;

    public DisableTableExecutor(TableStateManager stateManager) {
    ...
    public CompletableFuture<HBaseRpcResponse> execute(HBaseRpcRequest request) {
        try {
            ...
            MasterProtos.DisableTableRequest disableRequest =
                    MasterProtos.DisableTableRequest.parseFrom(requestBytes);

            HBaseProtos.TableName tableName = disableRequest.getTableName();
            String namespace =
            ...
            stateManager.disableTable(hbaseTableName);

            LOG.info("[DisableTable] Disabled table: {}.{}", namespace, qualifier);

            MasterProtos.DisableTableResponse response =
                    MasterProtos.DisableTableResponse.newBuilder().setProcId(1L).build();
            return CompletableFuture.completedFuture(
                    HBaseRpcResponse.success(request.getCallId(), response));

        } catch (Exception e) {
            LOG.error("[DisableTable] Failed to parse request", e);
            return CompletableFuture.completedFuture(
