...
public class TableNameExtractor {
    ...
    public static String extractTableName(String methodName, byte[] requestBytes) {
        ...
        try {
            switch (methodName) {
                case "CreateTable":
                    MasterProtos.CreateTableRequest createRequest =
                            MasterProtos.CreateTableRequest.parseFrom(requestBytes);
                    if (createRequest.hasTableSchema() && createRequest.getTableSchema().hasTableName()) {
                        HBaseProtos.TableName tableName = createRequest.getTableSchema().getTableName();
                        String extracted = extractFromTableName(tableName);
                        LOG.info("[EXTRACTOR] Extracted from CreateTable: {}", extracted);
                        return extracted;
                    }
                    break;
                case "DeleteTable":
                    MasterProtos.DeleteTableRequest deleteRequest =
                            MasterProtos.DeleteTableRequest.parseFrom(requestBytes);
                    if (deleteRequest.hasTableName()) {
                        String extracted = extractFromTableName(deleteRequest.getTableName());
                        LOG.info("[EXTRACTOR] Extracted from DeleteTable: {}", extracted);
                        return extracted;
                    }
                    break;
                case "Get":
    ...
    private static String extractFromTableName(HBaseProtos.TableName tableName) {
        if (tableName.hasQualifier()) {
            return new String(tableName.getQualifier().toByteArray(), StandardCharsets.UTF_8);
        }
        return null;
    }
}
