...
public class RowKeyEncoder {
    ...
    public byte[] encodeRowKey(GenericRow row) {
        if (primaryKeyFields.size() == 1) {
            String fieldName = primaryKeyFields.get(0);
            int fieldIndex = getFieldIndex(fieldName);
            DataField field = rowType.getFields().get(fieldIndex);
            
            if (field.getType().getTypeRoot() == org.apache.fluss.types.DataTypeRoot.BYTES) {
    ...
    public GenericRow decodeRowKey(byte[] rowKey) {
        if (primaryKeyFields.size() == 1) {
            ...
            if (field.getType().getTypeRoot() == org.apache.fluss.types.DataTypeRoot.BYTES) {
                ...
                row.setField(fieldIndex, rowKey);
                return row;
            }
        }
    ...
    private byte[] encodeWithDelimiter(GenericRow row) {
        ...
        for (int i = 0; i < primaryKeyFields.size(); i++) {
            ...
            if (!row.isNullAt(fieldIndex)) {
                ...
                sb.append(escapeDelimiter(valueStr));
            }
        }
    ...
    private GenericRow decodeWithDelimiter(byte[] rowKey) {
        ...
        List<String> parts = splitEscaped(rowKeyStr);

        GenericRow row = new GenericRow(rowType.getFieldCount());

        for (int i = 0; i < Math.min(parts.size(), primaryKeyFields.size()); i++) {
            String fieldName = primaryKeyFields.get(i);
            int fieldIndex = getFieldIndex(fieldName);
            DataField field = rowType.getFields().get(fieldIndex);

            if (!parts.get(i).isEmpty()) {
                Object value = stringToFieldValue(unescapeDelimiter(parts.get(i)), field.getType());
    ...
    private String unescapeDelimiter(String value) {
    ...
    private List<String> splitEscaped(String value) {
