...
public class HBaseSaslAuthenticator {
    ...
    private long failureWindowStartMs;
    ...
    public byte[] evaluateResponse(byte[] token) throws AuthenticationException {
        ...
        if (now < backoffUntilMs) {
            throw new AuthenticationException(
                    "SASL authentication temporarily throttled");
        }
        if (failureWindowStartMs == 0 || now - failureWindowStartMs > FAILURE_WINDOW_MS) {
